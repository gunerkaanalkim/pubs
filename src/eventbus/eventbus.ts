...
export default class Eventbus {
  ...
  public getSubscribersByTopic(topic: string): BusSubscribers {
  ...
  private _topicHasSameSubscriber(topic: string, subsciber: Subsciber) {
    let has = false;

    if (Array.isArray(topic)) {
      for (const tpc of topic) {
  ...
  private _publishToBus(topic: string, state: object, subscribers: object): void {
    this._bus[topic] = { state, subscribers };
  }
